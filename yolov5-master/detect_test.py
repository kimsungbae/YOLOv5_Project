...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
        ...
        line_thickness=30,  # bounding box thickness (pixels)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            im0 = imutils.resize(im0, width=1000) # 창 크기 변환
            rgb = cv2.cvtColor(im0, cv2.COLOR_BGR2RGB)  # BGR -> RGB 변환
            ...
            if len(det):  # 프레임에서 탐지된 객체 수(det은 객체별 정보0~3 위치, 4는 conf, 5은 객체 번호)
            ...
            if totalFrames % 30 == 0:
                ...
                for detection in det:  # detection에는 객체 하나의 정보(det)
                    ...
                    classID = int(detection[5].tolist())  # 객체 이름 정보 추출
                    confidence = float(detection[4].tolist()) # 객체 정확도 추출
                    # 마스크, 노마스크, error인 경우
                    # mask인 경우
            ...
            # 객체를 탐지하지 않을 때
                ...
                for f, tracker in enumerate(trackers1):
                    ...
                    cv2.putText(im0, f"{names[classIDs1[f]]}", (startX, startY), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color=(0, 255, 0),
                ...
                for f, tracker in enumerate(trackers2):
                    ...
                    # bounding box 출력
                    cv2.rectangle(im0, (startX, startY), (endX, endY), (255, 0, 0), 2)
                    cv2.putText(im0, f"{names[classIDs2[f]]}", (startX, startY), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color=(255, 0, 0),
                                thickness=fontThickness)
                ...
                for f, tracker in enumerate(trackers3):
                    ...
                    # bounding box 출력
                    cv2.rectangle(im0, (startX, startY), (endX, endY), (255, 255, 0), 2)
                    cv2.putText(im0, f"{names[classIDs3[f]]}", (startX, startY), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color=(255, 255, 0),
                                thickness=fontThickness)

            # 객체 중심 추적 / dict 형식으로 저장
